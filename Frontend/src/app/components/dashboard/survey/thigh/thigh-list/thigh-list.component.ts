...
import { ThighUpdateComponent } from '../thigh-update/thigh-update.component';
import { ThighAddComponent } from '../thigh-add/thigh-add.component';

@Component({
  selector: 'app-thigh-list',
  templateUrl: './thigh-list.component.html',
  styleUrls: ['./thigh-list.component.css']
})
export class ThighListComponent implements OnInit {
  ...
  deleteSurvey(id: number) {
    this.dashboardService.deleteThigh(id, this.authService.decodedToken.nameid).subscribe(()=>{
  ...
  editSurvey(element) {
    const dialogRef = this.dialog.open(ThighUpdateComponent, {
      width: '450px',
      data: element
    });
  ...
  addSurvey() {
    const dialogRef = this.dialog.open(ThighAddComponent, {
      width: '450px'
    });
