...
import { BodyFatAddComponent } from '../body-fat-add/body-fat-add.component';
import { BodyFatUpdateComponent } from '../body-fat-update/body-fat-update.component';

@Component({
  selector: 'app-body-fat-list',
  templateUrl: './body-fat-list.component.html',
  styleUrls: ['./body-fat-list.component.css']
})
export class BodyFatListComponent implements OnInit {
  ...
  public loadData() {
    ...
    .pipe(
      ...
      switchMap(() => {
        ...
        return this.dashboardService.bodyFatList(this.authService.decodedToken.nameid, this.listQuery);
      }),
      map(data => {         
  ...
  deleteSurvey(id: number) {
    this.dashboardService.deleteBodyFat(id, this.authService.decodedToken.nameid).subscribe(()=>{
  ...
  editSurvey(element) {
    const dialogRef = this.dialog.open(BodyFatUpdateComponent, {
      width: '450px',
      data: element
    });
  ...
  addSurvey() {
    const dialogRef = this.dialog.open(BodyFatAddComponent, {
      width: '450px'
    });
